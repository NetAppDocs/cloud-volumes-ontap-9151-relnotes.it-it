...
+
È possibile scegliere di non utilizzare l'efficienza dello storage quando si crea un volume da BlueXP  oppure è possibile creare un volume e quindi http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-vsmg/GUID-8508A4CB-DB43-4D0D-97EB-859F58B29054.html["Disattivare la compressione dei dati utilizzando l'interfaccia CLI"^].
...
Cloud Volumes ONTAP non rilascia CPU quando è inattivo, pertanto https://aws.amazon.com/cloudwatch/["Amazon CloudWatch"^] può segnalare un avviso di CPU elevata per l'istanza EC2 perché rileva un utilizzo del 100%. È possibile ignorare questo allarme. Il comando ONTAP statistics (Statistiche CPU) visualizza l'utilizzo effettivo delle CPU.
